...
%     fn_1=fn(1:16);
%     fn_3=[fn_1 '_R_EMG_nostim'];
%     load([fn_3 '.mat'])
%     
%     fn_4=[fn_1 '_R_Loco_nostim'];
%     load([fn_4 '.mat'])
...
%      
%      R_loco_positive=R_Loco_nostim(index_positive_1);
%      R_loco_negative=R_Loco_nostim(index_negative_1);
%      R_loco_unchanged=R_Loco_nostim(index_unchanged_1);
...
%classify IN neurons
 for ii=1:length(M)
    ...
    for m=1:size(temp,1)
        ...
        if (temp_1_CR-temp_1_base)>=4*temp_1_base_std & temp_1_UR_min>0
            temp_id(m)=1;
        elseif temp_1_UR_min<=0 & (abs(temp_1_UR_min)-temp_1_base)>=4*temp_1_base_std
            temp_id(m)=-1;
        else
            temp_id(m)=0;
        end
...
%     Index_1=Index{ii};
    ...
    temp_positive=temp_positive(:,6:53)+2.5*(ii-1);
    temp_negative=temp_negative(:,6:53)+2.5*(ii-1);
...
hold on
line([24,24],[2.5 7],'color','k','linestyle',':','linewidth',1);
...
xlim([0.5,length(M)+0.5]); ylim([0,2.1]); xticks([1:4]); xticklabels(stages); ylabel('Mean \Deltaf/f');
...
xlim([0.5,length(M)+0.5]); ylim([-0.8,0.5]); xticks([1:4]); xticklabels(stages); ylabel('Mean \Deltaf/f');
